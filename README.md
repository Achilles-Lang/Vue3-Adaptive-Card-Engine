...
无需修改任何组件代码。仅改变 `data` 字段，就能让同一张 `ChartCard` 分别展示：

| 场景 | 数据 | 效果 |
|:---|:---|:---|
| 季度营收对比 | `labels: ["Q1","Q2"], values: [100,200]` | 简洁的两柱对比 |
| 六周迭代趋势 | `labels: ["W1"..."W6"], values: [14,18,22,19,25,30]` | 完整的趋势折线图 |
| 7 天服务器延迟 | `labels: ["Mon"..."Sun"], values: [4.2,3.8,...]` | 运维监控曲线 |

同一张 `MetricCard`，指标数从 3 个变成 5 个——自动适配网格；同一张 `TodoCard`，从 3 项待办到 8 项——自动撑开列表；同一张 `ProfileCard`，一次渲染一张是名片，三次渲染三张就是团队成员目录。

**这些效果不需要新增一行组件代码，全部由数据控制。**

### 类型组合，效果乘数级放大

将 10 种卡片类型任意排列组合，就能拼出丰富的页面布局：

| 组合 | 效果 |
|:---|:---|
| `text` + `metric` + `chart` + `todo` + `progress` | 完整的项目健康报告 |
| `metric` + `chart` + `text` | 业务仪表盘 |
| `schedule` + `todo` + `image` | 旅行规划页 |
| `profile` × 3 | 团队成员目录 |
| `code` + `text` + `todo` | 技术文档 + 任务清单 |
| `metric` + `chart` + `text` | 运维监控看板 |

同一种卡片类型 × 数据变化 = 10+ 种效果，10 种类型 × 任意排列 = 数百种组合。这正是引擎核心理念 "Data + ComponentMap = UI" 的完整体现——**前端只写一次组件，数据决定一切界面。**
